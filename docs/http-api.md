...
| `POST` | `/api/v1/actions/wake` | 需要 | 跳过当前等待，立即触发一次轮询；不修改设置或机器人权限。 |
...
`status: "ok"` 不测量人工标准上的准确率。没有可领取任务时，`workerRunning: true` 与 `activeTasks: 0` 可以同时出现，不能据此宣称全部题目已验完。
...
| 字段 | JSON 类型和范围 | 说明 |
| --- | --- | --- |
| `enabled` | boolean | 是否启用自动领取；仍检查配置版本、档位、凭据及 Urmotiv 机器人权限。 |
...
该动作只要求当前调度器尽快开始一次轮询；它不会修改 `enabled`，不会增加并发上限，不会绕过 `experimentVersion`、模型凭据或机器人权限检查。服务处于禁用状态时，动作仍可返回成功，但不会领取任务。
...
- 启用条件和正式评分流程：[`src/reviewer-activation.ts`](../src/reviewer-activation.ts)、[`src/scorer.ts`](../src/scorer.ts)
