...
| 流水线 | 状态 | 说明 |
| --- | --- | --- |
| CF 难度（difficulty.ts） | **完整基线已取得，候选量尺待复测** | 修改前的 public83 v4 已完整处理 83/83，MAE 为 302.4、±200 命中率为 56.6%，未达到 MAE ≤ 200、命中率 ≥ 75% 的门槛；高分段有系统性低估。当前提示词加入了覆盖 800–3500 的分档量尺，但还没有候选报告，不能宣称准确性提升。`config/anchors/difficulty.json` 仍只有 2 条手工种子且标记为 `provisional: true`。 |
