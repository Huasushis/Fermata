...
本文保留离线实验操作与旧研究记录。返回 [Fermata 使用说明](../README.md)。

## 实验怎么跑

以下是独立准确性实验与历史研究记录，不是部署或开启正式服务的前置步骤。
正式服务入口是 `src/index.ts`，评分路径是 `src/scorer.ts`；不要把旧实验的资格状态
等同于当前服务可否运行，也不能用接口成功宣称准确性达标。
