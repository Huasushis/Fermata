...
function setup(output: unknown) {
	...
	const model: PipelineModelConfig = {
		...
		runtime: {
			fetch,
			firstOutputTimeoutMs: 600000,
			outputIdleTimeoutMs: 600000,
...
const { expectedRound: _round, verdict: _verdict, ...reviewFields } = review;
const modelReview = {
	...reviewFields,
	technicalVerdict: _verdict,
	tasteVerdict: "recommend",
	firstImpression: "清楚直接的合成构思。",
	tasteReason: "核心自然，符合本轮口味。",
};

describe("正式评分器", () => {
	it("学校接口 deepseek-flash 别名也发送 enabled/max，整理轮关闭思考", async () => {
		const { fetch, model } = setup(modelReview);
		const configured = resolveRuntimeModel(
			appConfig,
			{
				...settings,
				model: {
					baseUrl: "https://school.example.test/v1",
					model: "deepseek-flash",
					temperature: 0.1,
					thinking: true,
				},
			},
			{ modelApiKey: "synthetic" },
		)!;
		...
		expect(JSON.parse(fetch.mock.calls[1]![1].body).thinking).toEqual({
			type: "disabled",
		});
		expect(
			JSON.parse(fetch.mock.calls[1]![1].body).reasoning_effort,
		).toBeUndefined();
	});
	it("网页模型配置实际进入两轮 HTTP 请求，DeepSeek 使用 enabled/max", async () => {
		...
		for (const [url, init] of fetch.mock.calls) {
			expect(String(url)).toBe(
				"https://configured.example.test/v1/chat/completions",
			);
			expect(new Headers(init.headers).get("Authorization")).toBe(
				"Bearer configured-synthetic-key",
			);
	...
	it("两轮处理：深度思考 max 审题，再使用 JSON Schema 格式化；不需要查重资料", async () => {
		...
		expect(result.publicComment).toContain("本轮审题口味");
		...
		const target = JSON.parse(
			format.messages[0].content.split("完整 JSON Schema：\n")[1],
		);
		...
		expect(format.messages.at(-1).content).toBe(
			"合成审核结论，不代表准确性标定。",
		);
	});
	it.each([
	...
	it("题目快照错误时不发送模型请求", async () => {
		const { model, fetch } = setup(modelReview);
		await expect(
			scoreReviewTask({ ...task(), assignmentId: "invalid" }, model, []),
		).rejects.toThrow();
		expect(fetch).not.toHaveBeenCalled();
	});
	it("技术正确但明确不喜欢时拒绝，公开区分口味和技术判断", async () => {
		const { model } = setup({
			...modelReview,
			technicalVerdict: "approve",
			tasteVerdict: "decline",
			tasteReason: "分类讨论压过关键洞察，个人不喜欢这种实现负担。",
		});
		const result = await scoreReviewTask(task(), model, []);
		expect(result.verdict).toBe("reject");
		expect(result.publicComment).toContain("基础方案成立");
		expect(result.publicComment).toContain("不推荐采用");
	});
	it.each(["reject", "request_changes"] as const)(
		"喜欢也不能覆盖技术结论 %s",
		async (technicalVerdict) => {
			const { model } = setup({
				...modelReview,
				technicalVerdict,
				tasteVerdict: "recommend",
			});
			expect((await scoreReviewTask(task(), model, [])).verdict).toBe(
				technicalVerdict,
			);
		},
	);
	it("不使用失效或属于旧题面版本的附加资料", async () => {
		...
		const item = {
			...
			summary: "合成资料",
			data: {},
			contentHash: input.problem.contentHash,
			createdAt: new Date().toISOString(),
			expiresAt: null,
		};
		input.reviewItems.push(
			item,
			{ ...item, id: "old", contentHash: "b".repeat(64) },
			{ ...item, id: "expired", expiresAt: new Date(0).toISOString() },
		);
		await scoreReviewTask(input, model, []);
		const request = JSON.parse(fetch.mock.calls[0]![1].body);
		expect(
			JSON.parse(request.messages.at(-1).content).reviewItems.map(
